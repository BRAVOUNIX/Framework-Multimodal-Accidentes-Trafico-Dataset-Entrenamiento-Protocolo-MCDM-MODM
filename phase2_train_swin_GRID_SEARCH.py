"""
FASE 2: Grid Search para Swin Transformer Clasificación
========================================================
Plataforma: RunPod
Búsqueda exhaustiva de hiperparámetros para clasificación con Swin Transformer
...
import timm  # Para Swin Transformer
import numpy as np
...
def setup_logging(experiment_name):
    ...
    log_file = f"phase2_swin_grid_{experiment_name}_{timestamp}.log"
    ...
    logger = logging.getLogger('GridSearch_Swin')
...
class Config:
    """Configuración del grid search para Swin Transformer"""
    ...
    MODELS_DIR = BASE_DIR / "models/phase2_classification/swin"
    ...
    DEFAULT_HYPERPARAMS = {
        'imgsz': [224],  # Swin Transformer usa 224x224
        'lr0': [0.0001],  # LR más bajo para transformers
        'weight_decay': [0.05],
        'optimizer': ['AdamW'],  # AdamW es mejor para transformers
        'model_variant': ['swin_base_patch4_window7_224'],  # Swin-B
        'drop_path_rate': [0.2],
        'label_smoothing': [0.1],
        'pretrained': [True],
        'device': [0]
...
class DatasetManager:
    ...
    def create_limited_dataset(self, image_limit):
        ...
        temp_dir = self.config.OUTPUT_DIR / f"temp_limited_swin_dataset_{image_limit}"
...
def get_data_transforms(image_size=224):
    """Transformaciones - Swin Transformer usa 224x224"""
    data_transforms = {
...
def create_swin_model(num_classes, model_variant='swin_base_patch4_window7_224', 
                      pretrained=True, drop_path_rate=0.2):
    """Crea modelo Swin Transformer usando timm"""
...
class GridSearchEngine:
    """Motor de Grid Search para Swin Transformer"""
    ...
    def train_combination(self, combo_id, hyperparams, data_path, num_classes, class_names):
        ...
        try:
            self.logger.info(f"\n{'='*80}")
            self.logger.info(f"COMBINACIÓN {combo_id}: Swin Transformer")
            ...
            # Crear dataloaders
            batch_size = hyperparams.get('batch', 16)  # Swin usa batch más pequeño
            ...
            # Crear modelo
            model_variant = hyperparams.get('model_variant', 'swin_base_patch4_window7_224')
            
            self.logger.info(f"🏗️  Creando Swin Transformer ({model_variant})...")
            
            model = create_swin_model(
                num_classes=num_classes,
                model_variant=model_variant,
                pretrained=hyperparams.get('pretrained', True),
                drop_path_rate=hyperparams.get('drop_path_rate', 0.2)
            )
            ...
            lr = hyperparams.get('lr0', 0.0001)
            weight_decay = hyperparams.get('weight_decay', 0.05)
            ...
            # Cosine Annealing Scheduler (mejor para transformers)
                ...
                for phase in ['train', 'val']:
                    ...
                    for inputs, labels in pbar:
                        ...
                        with torch.set_grad_enabled(phase == 'train'):
                            # Swin Transformer siempre retorna Tensor (simple como ResNet50)
...
def main():
    parser = argparse.ArgumentParser(
        description='Grid Search para Swin Transformer Clasificación'
    )
    
    parser.add_argument('--mode', type=str, choices=['train', 'validate', 'both'],
                       default='both', help='Modo de ejecución')
    parser.add_argument('--experiment_name', type=str, default='grid_search_swin',
    ...
    config.MODELS_DIR = config.BASE_DIR / "models/phase2_classification/swin"
    
    # Setup logging
    logger, log_file = setup_logging(args.experiment_name)
    
    logger.info("="*80)
    logger.info("🚀 GRID SEARCH PARA SWIN TRANSFORMER CLASIFICACIÓN")
    logger.info("="*80)
    logger.info(f"\nExperimento: {args.experiment_name}")
    logger.info(f"Modelo: Swin Transformer")
