"""
FASE 2: Grid Search para InceptionV3 Clasificación
==================================================
Plataforma: RunPod
Búsqueda exhaustiva de hiperparámetros para clasificación con InceptionV3
...
def setup_logging(experiment_name):
    ...
    log_file = f"phase2_inception_grid_{experiment_name}_{timestamp}.log"
    ...
    logger = logging.getLogger('GridSearch_InceptionV3')
...
class Config:
    """Configuración del grid search para InceptionV3"""
    ...
    MODELS_DIR = BASE_DIR / "models/phase2_classification/inceptionv3"
    ...
    DEFAULT_HYPERPARAMS = {
        'imgsz': [299],  # InceptionV3 usa 299x299
        'lr0': [0.001],
        'momentum': [0.9],
        'weight_decay': [0.0001],
        'optimizer': ['SGD'],
...
class DatasetManager:
    ...
    def create_limited_dataset(self, image_limit):
        ...
        temp_dir = self.config.OUTPUT_DIR / f"temp_limited_inception_dataset_{image_limit}"
...
def get_data_transforms(image_size=299):
    """Transformaciones - InceptionV3 usa 299x299"""
    data_transforms = {
...
def create_dataloaders(data_path, batch_size, num_workers, imgsz=299, cache_mode='none'):
    """
    Crea dataloaders para train/val/test
    
    CORRECCIÓN: pin_memory=False para evitar ConnectionResetError
    cache_mode: 'none' o 'persistent' (simula disk cache con persistent_workers)
    """
...
def create_inceptionv3(num_classes, pretrained=True, freeze_layers=0, aux_logits=True):
    """Crea modelo InceptionV3"""
    ...
    # Modificar aux classifier si existe
    if aux_logits:
...
class GridSearchEngine:
    """Motor de Grid Search para InceptionV3"""
    ...
    def train_combination(self, combo_id, hyperparams, data_path, num_classes, class_names):
        ...
        try:
            self.logger.info(f"\n{'='*80}")
            self.logger.info(f"COMBINACIÓN {combo_id}: InceptionV3")
            ...
            imgsz = hyperparams.get('imgsz', 299)
            ...
            model = create_inceptionv3(
                ...
                aux_logits=hyperparams.get('aux_logits', True)
            )
                ...
                for phase in ['train', 'val']:
                    ...
                    for inputs, labels in pbar:
                        ...
                        with torch.set_grad_enabled(phase == 'train'):
                            # InceptionV3 con aux_logits retorna InceptionOutputs
                            if phase == 'train' and hyperparams.get('aux_logits', True):
                                # Durante training, InceptionV3 retorna InceptionOutputs
                                result = model(inputs)
                                
                                # Extraer logits y aux_logits del objeto InceptionOutputs
                                if hasattr(result, 'logits'):
                                    # Es un InceptionOutputs
                                    outputs = result.logits
                                    aux_outputs = result.aux_logits
                                    # Calcular pérdida combinada
                                    loss1 = criterion(outputs, labels)
                                    loss2 = criterion(aux_outputs, labels)
                                    loss = loss1 + 0.4 * loss2
                                else:
                                    # Fallback: es un tensor o tupla
                                ...
                                # Modo eval o sin aux_logits
                                outputs = model(inputs)
                                # Si outputs es InceptionOutputs, extraer logits
                                if hasattr(outputs, 'logits'):
                                    outputs = outputs.logits
                                loss = criterion(outputs, labels)
                            
                            # Asegurar que outputs es un tensor para max()
                            if hasattr(outputs, 'logits'):
                                outputs = outputs.logits
...
def main():
    parser = argparse.ArgumentParser(
        description='Grid Search para InceptionV3 Clasificación'
    )
    
    parser.add_argument('--mode', type=str, choices=['train', 'validate', 'both'],
                       default='both', help='Modo de ejecución')
    parser.add_argument('--experiment_name', type=str, default='grid_search_inception',
    ...
    config.MODELS_DIR = config.BASE_DIR / "models/phase2_classification/inceptionv3"
    
    # Setup logging
    logger, log_file = setup_logging(args.experiment_name)
    
    logger.info("="*80)
    logger.info("🚀 GRID SEARCH PARA INCEPTIONV3 CLASIFICACIÓN")
    logger.info("="*80)
    logger.info(f"\nExperimento: {args.experiment_name}")
    logger.info(f"Modelo: InceptionV3")
