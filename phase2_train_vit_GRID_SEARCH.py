"""
FASE 2: Grid Search para Vision Transformer (ViT) Clasificación
================================================================
Plataforma: RunPod
Búsqueda exhaustiva de hiperparámetros para clasificación con ViT
...
import timm  # Para Vision Transformer
import numpy as np
...
def setup_logging(experiment_name):
    ...
    log_file = f"phase2_vit_grid_{experiment_name}_{timestamp}.log"
    ...
    logger = logging.getLogger('GridSearch_ViT')
...
class Config:
    """Configuración del grid search para Vision Transformer"""
    ...
    MODELS_DIR = BASE_DIR / "models/phase2_classification/vit"
    ...
    DEFAULT_HYPERPARAMS = {
        'imgsz': [224],  # ViT usa 224x224
        'lr0': [0.0001],  # LR bajo para transformers
        'weight_decay': [0.05],
        'optimizer': ['AdamW'],  # AdamW para transformers
        'model_variant': ['vit_base_patch16_224'],  # ViT-B/16
        'drop_rate': [0.1],
        'label_smoothing': [0.1],
        'pretrained': [True],
        'device': [0]
...
class DatasetManager:
    ...
    def create_limited_dataset(self, image_limit):
        ...
        temp_dir = self.config.OUTPUT_DIR / f"temp_limited_vit_dataset_{image_limit}"
...
def get_data_transforms(image_size=224):
    """Transformaciones - ViT usa 224x224"""
    data_transforms = {
...
def create_vit_model(num_classes, model_variant='vit_base_patch16_224', 
                     pretrained=True, drop_rate=0.1):
    """Crea modelo Vision Transformer usando timm"""
...
class GridSearchEngine:
    """Motor de Grid Search para Vision Transformer"""
    ...
    def train_combination(self, combo_id, hyperparams, data_path, num_classes, class_names):
        ...
        try:
            self.logger.info(f"\n{'='*80}")
            self.logger.info(f"COMBINACIÓN {combo_id}: Vision Transformer")
            ...
            # Crear dataloaders
            batch_size = hyperparams.get('batch', 16)
            ...
            # Crear modelo
            model_variant = hyperparams.get('model_variant', 'vit_base_patch16_224')
            
            self.logger.info(f"🏗️  Creando Vision Transformer ({model_variant})...")
            
            model = create_vit_model(
                num_classes=num_classes,
                model_variant=model_variant,
                pretrained=hyperparams.get('pretrained', True),
                drop_rate=hyperparams.get('drop_rate', 0.1)
            )
                ...
                for phase in ['train', 'val']:
                    ...
                    for inputs, labels in pbar:
                        ...
                        with torch.set_grad_enabled(phase == 'train'):
                            # ViT siempre retorna Tensor (simple como ResNet50/Swin)
...
def main():
    parser = argparse.ArgumentParser(
        description='Grid Search para Vision Transformer Clasificación'
    )
    
    parser.add_argument('--mode', type=str, choices=['train', 'validate', 'both'],
                       default='both', help='Modo de ejecución')
    parser.add_argument('--experiment_name', type=str, default='grid_search_vit',
    ...
    config.MODELS_DIR = config.BASE_DIR / "models/phase2_classification/vit"
    
    # Setup logging
    logger, log_file = setup_logging(args.experiment_name)
    
    logger.info("="*80)
    logger.info("🚀 GRID SEARCH PARA VISION TRANSFORMER CLASIFICACIÓN")
    logger.info("="*80)
    logger.info(f"\nExperimento: {args.experiment_name}")
    logger.info(f"Modelo: Vision Transformer")
